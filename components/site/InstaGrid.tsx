      ...
      {images.map((image, index) => (
        ...
        >
          <Image
            ...
            width={400}
            height={400}
            className="h-full w-full object-cover transition duration-300 ease-luxurious group-hover:scale-105"
          />
          <span className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/40 via-black/0 to-transparent opacity-0 transition-opacity duration-300 ease-luxurious group-hover:opacity-100" />
        </Link>
      ))}
    </div>
  );
